...
function check_raylib()
    local raylib_dir = "raylib-" .. RAYLIB_VERSION
    if(os.isdir("raylib") == false and os.isdir(raylib_dir) == false) then
        local zip_name = raylib_dir .. ".zip"
        if(not os.isfile(zip_name)) then
            print("Raylib not found, downloading pinned release " .. RAYLIB_VERSION .. " from github")
            local result_str, response_code = http.download("https://github.com/raysan5/raylib/archive/refs/tags/" .. RAYLIB_VERSION .. ".zip", zip_name, {
